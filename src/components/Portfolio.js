...
const Preview = (props) => {
    ...
    return (
        <Box tag='article'
            ...
            border={{side: 'all', color: 'dark-2'}} round
            pad='small'
            style={{
                width: '100%',
                height: 'auto',
        ...
        {props.images.map(img => (
            ...
            {show && (
                <Layer
                    ...
                    margin='xlarge'>
                    <Box
                        pad='small' direction='column' flex
                        background='paper'>
                        ...
                        <Box pad='small' alignSelf='end'>
                            <Button
                                ...
                                color='logoGreen'
                                label='close' 
                                onClick={(e) => { setShow(false); returnYOffset(yOffSet)}} />
                        </Box>
                    </Box>
                </Layer>
            )}
    ...
    return (
        workContents.map(content => (
            <Box tag='section'
                ...
                margin={{ top: 'medium', bottom: 'medium' }}
                key={content.title}>
                <Heading level={1} textAlign='start' style={{ margin: 0 }}> 
                    {content.title}
                </Heading>
